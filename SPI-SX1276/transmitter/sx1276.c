...
char after_transmit[] = {0x92,0x08,0x81,0x80};
...
/*void readRegisters(){
    uint8_t RX = 0;
    char str[20];
    RX = sx1276_read(REG_FRFMSB);
    //sprintf(str, "REG_FRFMSB: %d", RX );
    //myUart_writeBuf( BACKCHANNEL_UART, (unsigned char *)str, NULL, CRLF );
    RX = sx1276_read(REG_FRFMID);
    //sprintf(str, "REG_FRFMID: %d", RX );
    //myUart_writeBuf( BACKCHANNEL_UART, (unsigned char *)str, NULL, CRLF );
    RX = sx1276_read(REG_FRFLSB);
    //sprintf(str, "REG_FRFLSB: %d", RX );
    //myUart_writeBuf( BACKCHANNEL_UART, (unsigned char *)str, NULL, CRLF );
    RX = sx1276_read(REG_OPMODE);
    //sprintf(str, "REG_OPMODE: %d", RX );
    //myUart_writeBuf( BACKCHANNEL_UART, (unsigned char *)str, NULL, CRLF );
}*/
...
void sx1276_send(uint8_t *buffer, uint8_t size) {
	...
	if(sx1276.Settings.LoRaPacketHandler.Size != size){
		sx1276_write(REG_LR_PAYLOADLENGTH, size); // Initializes the payload size
	}
  ...
  switch( sx1276.Settings.Modem ) {
    ...
    case MODEM_LORA: {
      ...
      sx1276_write(REG_LR_FIFOADDRPTR, 0);      // Has to be sent every time: Sentbufferaddress, return to 0
...
void sx1276_set_channel(uint32_t freq) {
    ...
    // For HF
    sx1276_write(REG_OPMODE, (sx1276_read(REG_OPMODE) & RFLR_OPMODE_FREQMODE_ACCESS_MASK) | RFLR_OPMODE_FREQMODE_ACCESS_HF);
}
...
void sx1276_write_buffer(uint8_t addr, uint8_t* data, uint8_t len) {
    ...
    spi_transfer(addr | 0x80);  // TX FIFO starts in addr = 0x80
