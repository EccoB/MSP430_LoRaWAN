...
char after_transmit[] = {0x92,0x08,0x81,0x80}; // Commands for SPIARRAY

void SX126xInit( RadioEvents_t *events )
{
    RadioEvents = events;
...
void SX126xSetTxParams( int8_t power, RadioRampTimes_t rampTime )
{
    ...
    {
        SX126xSetPaConfig( 0x02, 0x02, 0x00, 0x01 ); // Optimal PA Settings for +14dBm used, default is:(0x04, 0x07, 0x00, 0x01 );
        if( power > 22 )
        {
            power = 22;
        }
...
void SX126xGetRxBufferStatus( uint8_t *payloadLength, uint8_t *rxStartBufferPointer )
{
    ...
    // In case of LORA fixed header (implicit header), the payloadLength is obtained by reading
...
void sx126x_on_dioirq( void )
{
    ...
    if( ( irqRegs & IRQ_RX_DONE ) == IRQ_RX_DONE )
    {
        ...
        SX126xGetPayload( RxPayload, &size , 255 );
        SX126xGetPacketStatus( &PktStatus );
        if( ( RadioEvents != NULL ) && ( RadioEvents->RxDone != NULL ) )
        {
            RadioEvents->RxDone( RxPayload, size, PktStatus.Params.LoRa.RssiPkt, PktStatus.Params.LoRa.SnrPkt );
        }
    }
    ...
    if( ( irqRegs & IRQ_CRC_ERROR ) == IRQ_CRC_ERROR )
    {
        ...
        //myUart_writeBuf( BACKCHANNEL_UART, "CRC Error", NULL, CRLF ); //###
    }
    ...
    {
        //__NOP( );
        //myUart_writeBuf( BACKCHANNEL_UART, "Preamble Detected", NULL, CRLF ); //###
    }

    if( ( irqRegs & IRQ_SYNCWORD_VALID ) == IRQ_SYNCWORD_VALID )
    {
        //__NOP( );
        //myUart_writeBuf( BACKCHANNEL_UART, "Syncword Valid", NULL, CRLF ); //###
    }

    if( ( irqRegs & IRQ_HEADER_VALID ) == IRQ_HEADER_VALID )
    {
        //__NOP( );
        //myUart_writeBuf( BACKCHANNEL_UART, "Header Valid", NULL, CRLF ); //###
    }
    ...
    if( ( irqRegs & IRQ_HEADER_ERROR ) == IRQ_HEADER_ERROR )
    {
        ...
        //myUart_writeBuf( BACKCHANNEL_UART, "Header Error", NULL, CRLF ); //###
    }
}
